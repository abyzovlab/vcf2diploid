	...
	public Variant parseLine() {
		...
		while (toks.hasMoreTokens()) {
			...
			// INFO = toks.nextToken();
				...
				StringTokenizer words = new StringTokenizer(toks.nextToken(), ":");
		...
		if (ALT.equals("<DEL>")) {
			System.out.println("Skipping record with ALT = <DEL>");
			return null; // Imprecise SV
		}
		if (_pass && FILTER.indexOf("PASS") < 0) {
			System.out.println("Skipping record with FILTER = " + FILTER);
